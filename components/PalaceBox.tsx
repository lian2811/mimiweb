  ...
  const PalaceBox: React.FC<PalaceBoxProps> = ({ palace, index, isLife }) => {
    ...
    return (
        ...
        className={`border border-gray-300 rounded-lg ${isMing ? 'bg-gradient-to-br from-pink-100/90 to-purple-100/90' : palace.isBodyPalace ? 'bg-gradient-to-br from-indigo-100/90 to-blue-100/90' : 'bg-gradient-to-br from-white/90 to-gray-100/90'} 
                    p-2 h-full w-full flex flex-col justify-between text-xs shadow-sm
                    transition-all duration-200 transform hover:scale-105 hover:shadow-xl cursor-pointer
                    backdrop-blur-sm`}
        >
          {/* 星曜區（移除捲動） */}
          <div className="flex flex-wrap gap-1 overflow-hidden pr-1">
            {allStars.length ? (
              allStars.map((star, i) => (
                <span
                  key={i}
                  className={`rounded px-1 py-0.5 shadow-sm ${getStarClass(star.name)}`}
                  title={`${star.name}${star.brightness ? `(${star.brightness})` : ''}${star.mutagen ? ` [${star.mutagen}]` : ''}`}
                >
                  {star.name}
                  {star.mutagen && <span className="text-[9px] text-purple-500">【{star.mutagen}】</span>}
                  {star.isReverse && <sup className="text-orange-500">R</sup>}
                  {star.brightness && <sub className="text-violet-500">{star.brightness}</sub>}
                </span>
              ))
            ) : (
              <span className="text-gray-400 italic">無星曜</span>
            )}
          </div>
      
          {/* 底部資訊區 */}
          <div className="text-center mt-2">
            <div className={`${isMing ? 'text-pink-600' : palace.isBodyPalace ? 'text-indigo-600' : 'text-teal-600'} font-semibold text-base`}>
              {palace.name}
              {isMing && <span className="ml-1 text-pink-500">★</span>}
              {palace.isBodyPalace && <span className="ml-1 text-indigo-500">☯</span>}
            </div>
            <div className="text-gray-500 text-xs">{palace.heavenlyStem}{palace.earthlyBranch}</div>
            <div className="text-[10px] text-gray-500 mt-1 space-y-[1px]">
              ...
              {(palace.isBodyPalace || isMing) && (
                <div className="flex justify-center gap-2 mt-1">
                  {isMing && <span className="bg-gradient-to-r from-pink-400/80 to-purple-400/80 text-white px-2 py-0.5 rounded-full text-[9px]">命宮</span>}
                  {palace.isBodyPalace && <span className="bg-gradient-to-r from-indigo-400/80 to-blue-400/80 text-white px-2 py-0.5 rounded-full text-[9px]">身宮</span>}
                </div>
              )}
            </div>
          </div>
