    ...
    <div className="h-full flex flex-col overflow-x-hidden overflow-y-auto bg-gradient-to-br from-purple-800/30 to-indigo-900/30 backdrop-blur-lg rounded-xl shadow-xl">
      ...
      {chartData && (
        <div className="px-4 py-2 bg-green-600/20 border-b border-green-500/30 text-green-300 text-sm flex items-center justify-between">
          <span>
            <span className="font-semibold">命盤資料已載入</span> - AI 助手可以為您解讀
          </span>
      ...
      >
        {messages.map((message) => (
          <div key={message.id} className={`flex ${message.role === 'user' ? 'justify-end px-4 mb-3' : 'justify-start px-4 mb-4'}`}>
            <div 
        ...
        {thinkingStatus === 'waiting' && (
          <div className="flex justify-start px-4 my-2">
            <div className="max-w-[80%] p-4 rounded-xl bg-white/10 backdrop-blur-sm text-white">
        ...
        {thinkingStatus === 'thinking' && (
          <div className="flex justify-start px-4 my-2">
            <div className="max-w-[80%] p-4 rounded-xl bg-white/10 backdrop-blur-sm text白">
        ...
        {thinkingStatus === 'responding' && currentAssistantMessage && (
          <div className="flex justify-start px-4">
            <div className="max-w-[80%] p-4 rounded-xl bg白/10 backdrop-blur-sm text白">
              <div className="prose prose-invert prose-sm max-w-none whitespace-pre-wrap">
      ...
      <div className="p-3 border-t border-white/10">
        <form onSubmit={handleSubmit} className="relative">
          <textarea
            ...
            placeholder="輸入訊息..."
            className="w-full bg-white/10 backdrop-blur-sm text白 rounded-xl p-3 pr-12 resize-none min-h-[44px] max-h-[120px] focus:outline-none focus:ring-2 focus:ring-pink-500/50"
            disabled={isLoading}
          />
          <div className="absolute right-3 bottom-3 flex items-center space-x-2">
            <button type="button" className="text-white/50 hover:text白 transition-colors" title="上傳檔案">
              <Paperclip size={18} />
            </button>
            <button type="button" className="text-white/50 hover:text白 transition-colors" title="語音輸入">
              <Mic size={18} />
            </button>
            <button type="submit" disabled={!input.trim() || isLoading} className={`rounded-full p-1 ${!input.trim() || isLoading ? 'bg-pink-500/50 text-white/50' : 'bg-pink-500 text白 hover:bg-pink-600'} transition-colors`} title="發送">
              <Send size={18} />
            </button>
          </div>
        </form>
      ...
      <div className="px-3 pb-3"></div>
        ...
        <div className="flex flex-wrap gap-2 mt-2">
          {["我是1990年6月15日子時出生的男生，請幫我分析命盤", "幫我看這盤", "紫微與天府同宮的意義?"].map((q, i) => (
            <button key={i} onClick={() => { setInput(q); if (inputRef.current) inputRef.current.focus(); }} className="text-xs bg-white/10 hover:bg白/20 text白/90 px-3 py-1.5 rounded-full transition-colors">
              {q}
            </button>
          ))}
        </div>
