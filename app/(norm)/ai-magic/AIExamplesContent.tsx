...
export default function AIExamplesContent({
  ...
  const examples: Example[] = [
    ...
    {
      id: 'mimi-chat',
      title: t('examples.mimiChat.title') || 'Mimi AI Chat',
      description: t('examples.mimiChat.description') || '智能 AI 聊天助手，回答問題、創建內容',
      icon: <span className="text-2xl">🤖</span>,
      component: (
        <div className="h-full flex items-center justify-center p-4">
          <Link 
            href="/mimi-chat" 
            className="flex items-center justify-center bg-gradient-to-r from-[#6366f1] to-[#a855f7] text-white font-bold py-3 px-6 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 text-lg"
          >
            {t('examples.mimiChat.buttonText') || '立即體驗 Mimi Chat'}
            <ChevronRight className="ml-2 h-5 w-5" />
          </Link>
        </div>
      )
