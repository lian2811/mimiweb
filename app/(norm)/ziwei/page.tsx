...
import ZiweiChart from '@/app/(norm)/ziwei/ZiweiChart';
import ChatBox from '@/app/(norm)/ziwei/ChatBox';
  ...
  return (
    <div className="min-h-screen pt-24">
      <main className="relative pt-10 px-2 min-h-screen w-full bg-[var(--background)] text-[var(--foreground)]">
        <div className="flex flex-col md:flex-row h-full gap-4 max-w-[1440px] mx-auto">
            
            {/* ChatBot 區塊 - 使用 Gemini API 聊天室 */}
            <div className="flex-1 overflow-hidden h-[calc(100vh-180px)] md:h-[calc(100vh-140px)] order-1">
                <ChatBox 
                  chartData={chartData} 
            ...
            {/* 命盤顯示區塊 */}
            <div className="flex-1 bg-gradient-to-br from-purple-800/30 to-indigo-900/30 backdrop-blur-lg rounded-xl overflow-hidden p-1 h-[calc(100vh-180px)] md:h-[calc(100vh-140px)] flex flex-col order-2">
            {loading ? (
                ...
                <div className="flex-1 flex items-center justify-center text-gray-300">
                  <div className="text-center">
                    ...
                    <p className="text-sm opacity-80">你可以直接向AI助手詢問，<br />例如：「我是1990年6月15日子時出生的男生」</p>
                  </div>
                </div>
            )}
            </div>
            ...
            {/* 命盤表單 */}
            <div className="bg-white/30 backdrop-blur-lg rounded-xl shadow-xl text-white p-4 space-y-4 md:self-start order-3 mb-4 md:mb-0 w-full md:w-auto md:max-w-[300px]">
              <form onSubmit={handleSubmit} className="space-y-4">
                  <h1 className="text-3xl md:text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-[var(--primary)] to-[var(--secondary)]">
                    {t('title')}
                  </h1>
                  <div>
