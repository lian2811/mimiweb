        ...
        <div className="grid grid-cols-4 grid-rows-4 gap-1 h-full w-full">
          {palacePositions.map(({ palace, gridPosition}, i) =>
            palace ? (
              ...
              >
                {/* 傳入宮位資料和移動設備狀態 */}
                <PalaceBox palace={palace} index={i} isLife={palace.isOriginalPalace} isMobile={isMobile} />
              </div>
            ) : null
          )}

          {/* 中央區域，顯示其他詳細資料 */}
          <div className="col-start-2 row-start-2 col-span-2 row-span-2 bg-gradient-to-br from-pink-300/70 to-purple-400/70 backdrop-blur-sm rounded-lg text-center p-2 md:p-4 text-xs md:text-sm font-bold text-gray-700 flex flex-col justify-center shadow-md">
            <div>命主：{data.soul}</div>
            <div>身主：{data.body}</div>
            <div>五行局：{data.fiveElementsClass}</div>
            <div className="mt-2 text-gray-600 text-xs md:text-sm font-normal leading-tight md:leading-5">
              ...
              {/* 命宮與身宮特別樣式 */}
              <div className="mt-2 flex justify-center space-x-2 md:space-x-3">
                <div className="bg-gradient-to-r from-pink-500/70 to-purple-500/70 rounded-lg px-2 py-1 md:px-3 md:py-1 shadow-sm text-white text-xs md:text-sm">
                  命宮：{data.earthlyBranchOfSoulPalace}
                </div>
                <div className="bg-gradient-to-r from-indigo-500/70 to-blue-500/70 rounded-lg px-2 py-1 md:px-3 md:py-1 shadow-sm text-white text-xs md:text-sm">
